...
Надёжность:
  - Мультидомен: punycode (xn--80a7adb.xn--90adear.xn--p1ai) как основной,
    stat.gibdd.ru как запасной (для хостингов, блокирующих основной домен).
  - 3 попытки с задержкой 5/10/15с на каждый месяц перед пропуском.
  - Пропущенные месяцы возвращаются в списке errors для уведомления пользователя.
"""
